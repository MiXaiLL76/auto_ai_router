...
func aggregateDailyAgentSpendLogs(
	...
	if err != nil {
		logger.Error("[DB] Agent aggregation: failed to fetch spend logs", "error", err)
		return err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error("[DB] Agent aggregation: failed to scan row", "error", err)
			continue
		}
	...
	if rows.Err() != nil {
		logger.Error("[DB] Agent aggregation: failed to iterate rows", "error", rows.Err())
		return rows.Err()
	}
	...
	for key, value := range aggregations {
		_, err := conn.Exec(ctx,
			...
			key.agentID, key.date, key.apiKey, key.model,
