...
func TestDownloadSosanaResultImageRejectsUnsafeProductionURL(t *testing.T) {
	...
	image, _, statusCode, err := prx.downloadSosanaResultImage(context.Background(), cred, "banana-2-1k-compliant", sosanaBananaTaskResponse{
		Status:        sosanaStatusCompleted,
...
func TestSosanaImageGenerationRequest(t *testing.T) {
	tests := []struct {
		name       string
		size       string
		wantAspect string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, concreteModel, err := buildSosanaImageGenerationRequest(body, "banana-2-{image_size}-compliant")
			require.NoError(t, err)

			var req sosanaBananaCreateRequest
...
func TestSosanaImageGenerationRequestPrefersProviderModel(t *testing.T) {
	got, concreteModel, err := buildSosanaImageGenerationRequest([]byte(`{"model":"public-image","prompt":"draw","n":1}`), "banana-2-1k-compliant")
	require.NoError(t, err)
...
func TestSosanaImageGenerationRequestMapsPublicGeminiModelToSosanaTier(t *testing.T) {
	got, concreteModel, err := buildSosanaImageGenerationRequest(
...
func TestSosanaImageGenerationRequestUsesExplicitAspectRatio(t *testing.T) {
	got, _, err := buildSosanaImageGenerationRequest([]byte(`{"model":"banana-2-1k-compliant","prompt":"draw","size":"1024x1024","aspect_ratio":"16:9"}`), "banana-2-1k-compliant")
	require.NoError(t, err)
...
func TestSosanaImageGenerationRequestRejectsMultipleImages(t *testing.T) {
	_, _, err := buildSosanaImageGenerationRequest([]byte(`{"model":"banana-2-1k-compliant","prompt":"draw","n":2}`), "banana-2-1k-compliant")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "n=1")
}

func TestSosanaImageGenerationRequestRejectsURLResponseFormat(t *testing.T) {
	_, _, err := buildSosanaImageGenerationRequest([]byte(`{"model":"banana-2-1k-compliant","prompt":"draw","response_format":"url"}`), "banana-2-1k-compliant")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "response_format=url")
}

func TestSosanaImageGenerationRequestRejectsUnsupportedControls(t *testing.T) {
	tests := []struct {
		name string
		body string
		want string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, _, err := buildSosanaImageGenerationRequest([]byte(tt.body), "banana-2-1k-compliant")
			require.Error(t, err)
...
func TestSosanaImageGenerationRequestAllowsPNGOutputFormat(t *testing.T) {
	_, _, err := buildSosanaImageGenerationRequest([]byte(`{"model":"banana-2-1k-compliant","prompt":"draw","output_format":"png","response_format":"b64_json"}`), "banana-2-1k-compliant")
	require.NoError(t, err)
}

func TestSosanaImageEditRequest(t *testing.T) {
	body, contentType := sosanaMultipartEditBody(t, map[string]string{
		"model":  "banana-2-1k-compliant",
		"prompt": "make it blue",
		"size":   "1024x1024",
		...
		"image": sosanaResultPNG,
	})

	got, concreteModel, err := buildSosanaImageEditRequest(body, contentType, "banana-2-1k-compliant")
	require.NoError(t, err)
	...
	assert.Contains(t, req.ImageURLs[0], base64.StdEncoding.EncodeToString(sosanaResultPNG))
}

func TestSosanaImageEditRequestPrefersProviderModel(t *testing.T) {
	body, contentType := sosanaMultipartEditBody(t, map[string]string{
		"model":  "public-image",
		"prompt": "make it blue",
	}, map[string][]byte{
...
func TestSosanaImageEditRequestRejectsMask(t *testing.T) {
	body, contentType := sosanaMultipartEditBody(t, map[string]string{
		"model":  "banana-2-1k-compliant",
		"prompt": "make it blue",
	}, map[string][]byte{
		"image": sosanaResultPNG,
		"mask":  sosanaResultPNG,
	})

	_, _, err := buildSosanaImageEditRequest(body, contentType, "banana-2-1k-compliant")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "mask")
}

func TestSosanaImageEditRequestRejectsMultipleImagesCount(t *testing.T) {
	body, contentType := sosanaMultipartEditBody(t, map[string]string{
		"model":  "banana-2-1k-compliant",
		"prompt": "make it blue",
		"n":      "2",
...
func TestSosanaImageEditRequestRejectsURLResponseFormat(t *testing.T) {
	body, contentType := sosanaMultipartEditBody(t, map[string]string{
		"model":           "banana-2-1k-compliant",
		"prompt":          "make it blue",
		"response_format": "url",
...
func TestSosanaImageEditRequestRejectsJPEGInput(t *testing.T) {
	body, contentType := sosanaMultipartEditBody(t, map[string]string{
		"model":  "banana-2-1k-compliant",
		"prompt": "make it blue",
	}, map[string][]byte{
		"image": {0xff, 0xd8, 0xff, 0xdb, 0, 0x43, 0, 1, 2, 3},
	})
...
func TestSosanaImageEditRequestRejectsTooManyImages(t *testing.T) {
	...
	for i := 0; i < maxSosanaInputImages+1; i++ {
		part, err := writer.CreateFormFile("image", fmt.Sprintf("image-%02d.png", i))
		require.NoError(t, err)
		_, err = part.Write(sosanaResultPNG)
		require.NoError(t, err)
	}
	require.NoError(t, writer.Close())

	_, _, err := buildSosanaImageEditRequest(buf.Bytes(), writer.FormDataContentType(), "banana-2-1k-compliant")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "too many")
}

func TestSosanaOpenAIImageResponse(t *testing.T) {
	createdAt := "2026-01-01T00:00:00Z"
	body, err := buildSosanaOpenAIImageResponse(sosanaBananaTaskResponse{
		Status:          sosanaStatusCompleted,
		CreatedAt:       createdAt,
