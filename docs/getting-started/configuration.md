...
`tiktoken_enabled` controls the local prompt/completion token estimator used as a fallback for streaming responses when a provider doesn't report usage (e.g. the stream is cut before the final usage chunk, or the provider omits token counts entirely). The estimate is computed lazily — it only runs when it's actually needed, so leaving this on has no cost for providers that do report usage. Set to `false` to skip local estimation entirely (spend logs will show `0` prompt/completion tokens for the affected requests instead of an estimate) if all your configured providers always report usage and you'd rather not carry the `tiktoken-go` tokenization cost for the rare cases that need it.
